...
function JobDetails() {
  ...
  const displayTabContent =() =>{
    switch (activeTab) {
      ...
      case "About":
        return (
          <JobAbout info={data[0].job_description ?? "No data provided"} title="About the job:"/>
        );
  ...
  <SafeAreaView style={{flex:1,backgroundColor:COLORS.lightWhite}}>
   ...
   { data.length != 0 ?( <JobFooter url={data[0]?.job_google_link ?? 'https://careers.google.com/jobs/results'}/>):(<></>) }
    </>
